...
box=plt.boxplot(df['conversion_primer_respuesta'],patch_artist=True, notch=True, vert=True, showmeans=True, flierprops=flierprops)
...
#Etiquetas y titulo
plt.title('Distribucion de conversion primer respuesta por Anuncio', fontsize=16)
plt.ylabel('Promedio', fontsize=14)
plt.grid(True, linestyle='--', alpha=1)

#Ajustar el rango del eje 'Y' y los ticks
#plt.ylim(0, df['conversion_primer_respuesta'].max()+500)#ajustar el limite superior del eje Y
#plt.yticks(range(0, int(df['conversion_primer_respuesta'].max())+0.5, 5000)) #configurar los ticks de 100 en 100

#Mostrar grafico
plt.show()

# Identificar límites del boxplot
q1 = df['conversion_primer_respuesta'].quantile(0.25)
q3 = df['conversion_primer_respuesta'].quantile(0.75)
iqr = q3 - q1
lower_bound = q1 - 1.5 * iqr
upper_bound = q3 + 1.5 * iqr

# Filtrar anuncios con mas conversion_primer_respuesta por encima del límite superior
anuncios_outliers = df[df['conversion_primer_respuesta'] > upper_bound]

#ordenar los outliners de mayor a menor por 'conversion_primer_respuesta'
anuncios_outliers= anuncios_outliers.sort_values(by='conversion_primer_respuesta', ascending=False)

# Mostrar los nombres de los anuncios que son outliers
print("Conversion primer respuesta (outliers):")
