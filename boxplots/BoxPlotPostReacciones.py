# post_reaction
# - Número total de reacciones a la publicación (Me gusta, Me encanta, Me sorprende, etc.).
# - Evalúa el nivel de aceptación emocional del contenido entre la audiencia.
...
box=plt.boxplot(df['reacciones_post'],patch_artist=True, notch=True, vert=True, showmeans=True, flierprops=flierprops)
...
#Etiquetas y titulo
plt.title('Distribucion de reacciones en el post', fontsize=16)
plt.ylabel('reacciones', fontsize=16)
plt.grid(True, linestyle='--', alpha=1)

#Ajustar el rango del eje 'Y' y los ticks
plt.ylim(0, df['reacciones_post'].max()+500)#ajustar el limite superior del eje Y
plt.yticks(range(0, int(df['reacciones_post'].max())+500, 250)) #configurar los ticks de 100 en 100

#Mostrar grafico
plt.show()

# Identificar límites del boxplot
q1 = df['reacciones_post'].quantile(0.25)
q3 = df['reacciones_post'].quantile(0.75)
iqr = q3 - q1
lower_bound = q1 - 1.5 * iqr
upper_bound = q3 + 1.5 * iqr

# Filtrar anuncios con mas reacciones_post por encima del límite superior
anuncios_outliers = df[df['reacciones_post'] > upper_bound]

#ordenar los outliners de mayor a menor 
anuncios_outliers= anuncios_outliers.sort_values(by='reacciones_post', ascending=False)

# Mostrar los nombres de los anuncios que son outliers
print("Anuncios con más reacciones en el post (outliers):")
