# onsite_conversion.messaging_user_depth_3_message_send(3er msj cliente)
# - Mide cuántos usuarios han enviado un mensaje en el tercer nivel de una conversación dentro de la plataforma de mensajería.
# - Indica un mayor nivel de compromiso en la conversación.

#Boxplot para Columna 3er_msj_cliente de anuncios
...
box=plt.boxplot(df['3er_msj_cliente'],patch_artist=True, notch=True, vert=True, showmeans=True, flierprops=flierprops)
...
#Etiquetas y titulo
plt.title('Distribucion del 3er msj por cliente de anuncios', fontsize=16)
plt.ylabel('3er_msj_cliente ($)', fontsize=16)
plt.grid(True, linestyle='--', alpha=1)

#Ajustar el rango del eje 'Y' y los ticks
#plt.ylim(0, df['3er_msj_cliente'].max()+100)#ajustar el limite superior del eje Y
plt.yticks(range(0, int(df['3er_msj_cliente'].max())+100, 10)) #configurar los ticks de 100 en 100

#Mostrar grafico
plt.show()

# Identificar límites del boxplot
q1 = df['3er_msj_cliente'].quantile(0.25)
q3 = df['3er_msj_cliente'].quantile(0.75)
iqr = q3 - q1
