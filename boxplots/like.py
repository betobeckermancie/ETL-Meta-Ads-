...
#Boxplot para Columna like de anuncios
...
box=plt.boxplot(df['like'],patch_artist=True, notch=True, vert=True, showmeans=True, flierprops=flierprops)
...
#Etiquetas y titulo
plt.title('Distribucion de Me Gustas por Anuncios', fontsize=16)
plt.ylabel('like ($)', fontsize=16)
plt.grid(True, linestyle='--', alpha=1)

#Ajustar el rango del eje 'Y' y los ticks
plt.ylim(0, df['like'].max()+500)#ajustar el limite superior del eje Y
plt.yticks(range(0, int(df['like'].max())+500, 250)) #configurar los ticks de 100 en 100
