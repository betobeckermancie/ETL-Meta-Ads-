#Boxplot para Columna 5to_msj_cliente de anuncios
...
box=plt.boxplot(df['5to_msj_cliente'],patch_artist=True, notch=True, vert=True, showmeans=True, flierprops=flierprops)
...
#Etiquetas y titulo
plt.title('Distribucion del 5to msj de cliente', fontsize=16)
plt.ylabel('5to msj del cliente', fontsize=16)
plt.grid(True, linestyle='--', alpha=1)

#Ajustar el rango del eje 'Y' y los ticks
plt.ylim(0, df['5to_msj_cliente'].max()+500)#ajustar el limite superior del eje Y
plt.yticks(range(0, int(df['5to_msj_cliente'].max())+500, 250)) #configurar los ticks de 100 en 100

#Mostrar grafico
plt.show()

# Identificar límites del boxplot
q1 = df['5to_msj_cliente'].quantile(0.25)
q3 = df['5to_msj_cliente'].quantile(0.75)
iqr = q3 - q1
lower_bound = q1 - 1.5 * iqr
upper_bound = q3 + 1.5 * iqr

# Filtrar anuncios con mas 5to_msj_cliente por encima del límite superior
anuncios_outliers = df[df['5to_msj_cliente'] > upper_bound]

#ordenar los outliners de mayor a menor
anuncios_outliers= anuncios_outliers.sort_values(by='5to_msj_cliente', ascending=False)

# Mostrar los nombres de los anuncios que son outliers
print("Anuncios con el 5to msj cliente (outliers):")
