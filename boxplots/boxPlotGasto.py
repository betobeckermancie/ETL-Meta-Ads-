#Boxplot para Columna Gasto de anuncios
...
box=plt.boxplot(df['gasto'],patch_artist=True, notch=True, vert=True, showmeans=True, flierprops=flierprops)
...
#Etiquetas y titulo
plt.title('Distribucion del gasto por Anuncios', fontsize=16)
plt.ylabel('Gasto ($)', fontsize=16)
plt.grid(True, linestyle='--', alpha=1)

#Ajustar el rango del eje 'Y' y los ticks
plt.ylim(0, df['gasto'].max()+500)#ajustar el limite superior del eje Y
plt.yticks(range(0, int(df['gasto'].max())+500, 250)) #configurar los ticks de 100 en 100
...
# Filtrar anuncios con mas gasto por encima del límite superior
anuncios_outliers = df[df['gasto'] > upper_bound]

#ordenar los outliners de mayor a menor 
anuncios_outliers= anuncios_outliers.sort_values(by='gasto', ascending=False)

# Mostrar los nombres de los anuncios que son outliers
print("Anuncios con más gasto($) (outliers):")
