...
# - Mide el costo de las impresiones y es clave para analizar la rentabilidad en términos de visibilidad.
...
box=plt.boxplot(df['costo_por_mil_impresiones'],patch_artist=True, notch=True, vert=True, showmeans=True, flierprops=flierprops)
...
#Etiquetas y titulo
plt.title('Distribucion del costo por mil impresiones de Anuncio', fontsize=16)
plt.ylabel('Costo($)', fontsize=14)
plt.grid(True, linestyle='--', alpha=1)

#Ajustar el rango del eje 'Y' y los ticks
plt.ylim(0, df['costo_por_mil_impresiones'].max()+1)  # Ajustar el límite superior del eje Y
plt.yticks(range(0, int(df['costo_por_mil_impresiones'].max())+2, 1))  # Configurar los ticks de 1 en 1
...
q1 = df['costo_por_mil_impresiones'].quantile(0.25)
q3 = df['costo_por_mil_impresiones'].quantile(0.75)
iqr = q3 - q1
lower_bound = q1 - 1.5 * iqr
upper_bound = q3 + 1.5 * iqr

# Filtrar anuncios con mas costo_por_mil_impresiones por encima del límite superior
anuncios_outliers = df[df['costo_por_mil_impresiones'] > upper_bound]

#ordenar los outliners de mayor a menor por 'costo_por_mil_impresiones'
...
print("Costo por mil impresiones de anuncio (outliers):")
