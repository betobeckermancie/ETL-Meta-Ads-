...
# - Número de conversaciones que recibieron una respuesta dentro de los 7 días posteriores al inicio de la conversación.
# - Mide la reactividad de la audiencia a las interacciones iniciales.
#Boxplot para Columna msjs_respondidos_en_7Dias de anuncios
...
box=plt.boxplot(df['msjs_respondidos_en_7Dias'],patch_artist=True, notch=True, vert=True, showmeans=True, flierprops=flierprops)
...
#Etiquetas y titulo
plt.title('Distribucion de msjs respondidos en 7 Dias por Anuncios', fontsize=16)
plt.ylabel('msjs respondidos en 7Dias', fontsize=16)
plt.grid(True, linestyle='--', alpha=1)

#Ajustar el rango del eje 'Y' y los ticks
plt.ylim(0, df['msjs_respondidos_en_7Dias'].max()+500)#ajustar el limite superior del eje Y
plt.yticks(range(0, int(df['msjs_respondidos_en_7Dias'].max())+500, 250)) #configurar los ticks de 100 en 100

#Mostrar grafico
plt.show()

# Identificar límites del boxplot
q1 = df['msjs_respondidos_en_7Dias'].quantile(0.25)
q3 = df['msjs_respondidos_en_7Dias'].quantile(0.75)
iqr = q3 - q1
